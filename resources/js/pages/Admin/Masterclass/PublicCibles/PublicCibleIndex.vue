...
const props = defineProps<{ cibles: Cible[]; flash?: { success?: string; error?: string } }>();
...
const toDelete = ref<Cible | null>(null);
function doDelete() {
    if (toDelete.value) router.delete(route('admin.masterclass.public-cibles.destroy', toDelete.value.id));
    toDelete.value = null;
}
</script>

<template>
    <Head title="Masterclass — Public Cible" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-white shadow-sm">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b pb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Public Cible</h2>
                    <p class="text-gray-500 mt-1">Profils de participants affichés sur la page Masterclass</p>
                </div>
                <Link :href="route('admin.masterclass.public-cibles.create')"
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="c in props.cibles" :key="c.id"
                    class="group p-5 rounded-2xl border-2 border-gray-100 hover:border-blue-100 transition-all">
                    <div class="flex items-start gap-3 mb-4">
                        <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background: linear-gradient(135deg, #0B1437, #1E3A8A);">
                            <i :class="['bi', c.icon, 'text-white text-lg']"></i>
                        </div>
                        <div>
                            <div class="font-bold text-gray-800 text-sm">{{ c.titre }}</div>
                            <p class="text-xs text-gray-400 mt-1 leading-relaxed">{{ c.description }}</p>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Link :href="route('admin.masterclass.public-cibles.edit', c.id)" class="text-xs px-3 py-1 bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100">Modifier</Link>
                            <button @click="toDelete = c" class="text-xs px-3 py-1 bg-red-50 text-red-700 rounded-md hover:bg-red-100">Supprimer</button>
                        </div>
                    </div>
                </div>
                <div v-if="props.cibles.length === 0" class="col-span-3 py-10 text-center text-gray-400">Aucun profil.</div>
            </div>
        </div>
    </AppLayout>
    <div v-if="toDelete" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
            <h3 class="text-lg font-semibold mb-4 text-center">Supprimer ce profil ?</h3>
