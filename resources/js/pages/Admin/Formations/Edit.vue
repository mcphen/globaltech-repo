...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Modifier', href: route('admin.formations.edit', props.formation.id) }
];
...
const form = useForm({
  ...
  date_mode: (props.formation.date_mode as 'single' | 'range') || 'single',
  date: props.formation.date || '',
  start_time: props.formation.start_time || '',
  end_time: props.formation.end_time || '',
  start_date: props.formation.start_date || '',
  end_date: props.formation.end_date || '',
  duration_hours: (props.formation.duration_hours as any) || '' as any,
  duration_per_day_hours: (props.formation.duration_per_day_hours as any) || '' as any,
  price: (props.formation.price as any) || '' as any,
  currency: (props.formation.currency as string | null) ?? 'EUR',
  _method: 'PUT'
});

const imagePreview = ref<string | null>(props.formation.image_path ? `${props.formation.image_path}` : null);
...
onMounted(() => {
  if (editorRef.value) {
    ...
    if (props.formation.description) {
      editor.root.innerHTML = props.formation.description;
    }
  }
});
...
function submit() {
  ...
  form.post(route('admin.formations.update', props.formation.id), {
    forceFormData: true,
    preserveScroll: true
  });
}
...
<template>
  <Head :title="`Modifier la formation — ${props.formation.title}`" />
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 bg-white">
      <div class="flex items-center justify-between">
        <h2 class="text-2xl font-semibold">Modifier une formation</h2>
        <div class="flex items-center gap-2">
          <Link :href="route('admin.formations.show', props.formation.id)" class="px-3 py-2 border rounded">Voir</Link>
          <Link :href="route('admin.formations.index')" class="px-3 py-2 border rounded">Retour</Link>
        </div>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        <div>
          ...
          <input v-model="form.title" type="text" class="w-full px-3 py-2 border rounded" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div v-if="props.formation.attachment_path && !form.attachment" class="mt-2 text-sm">
              <a :href="`/storage/${props.formation.attachment_path}`" target="_blank" class="text-blue-600 underline">Brochure actuelle</a>
            </div>
          </div>
        </div>
