  ...
  <LayoutFront>
    ...
    <div class="relative bg-gray-900">
      <div class="absolute inset-0 overflow-hidden">
        <img src="/images/breadcrumb-bg.jpg" alt="Bannière Formations" class="w-full h-full object-cover object-center opacity-40">
      ...
      <div class="relative max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center">
        <h1 class="text-4xl md:text-5xl font-serif font-bold text-white text-center mb-4">Formations</h1>
      ...
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="lg:flex lg:items-center lg:justify-between lg:space-x-4">
          <div class="mb-4 lg:mb-0 lg:max-w-md lg:flex-grow">
            <div class="relative">
              <input v-model="searchQuery" type="text" placeholder="Rechercher une formation..." class="focus:ring-primary w-full rounded-full border border-gray-300 px-4 py-2 focus:border-transparent focus:ring-2 focus:outline-none" />
          ...
          <button @click="resetFilters" class="hover:text-primary hover:border-primary rounded-full border border-gray-300 px-4 py-2 text-sm text-gray-600 transition-colors">
            Réinitialiser
          </button>
        </div>
      </div>
    ...
    <section v-else class="bg-white py-12">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div v-if="formations.data.length === 0" class="py-16 text-center">
          ...
          <button @click="resetFilters" class="bg-primary hover:bg-primary-dark mt-4 rounded-full px-6 py-2 text-white transition-colors">Voir toutes les formations</button>
        </div>

        <div v-else class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          <div v-for="f in formations.data" :key="f.slug" class="group flex flex-col overflow-hidden rounded-lg bg-white shadow-md transition-shadow duration-300 hover:shadow-xl">
            <div class="relative h-56 overflow-hidden">
              <img :src="f.image_path ? `${f.image_path}` : '/images/placeholder.jpg'" :alt="f.title" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
            ...
            <div class="flex flex-grow flex-col p-5">
              <h2 class="group-hover:text-primary mb-1 font-serif text-xl font-semibold text-gray-800 transition-colors">{{ f.title }}</h2>
              <p v-if="f.price" class="mb-2 text-sm text-gray-700">
                <span class="font-medium">Tarif: </span>
                {{ Number(f.price).toLocaleString('fr-FR', { minimumFractionDigits: 0, maximumFractionDigits: 2 }) }} {{ f.currency || 'FCFA' }}
              </p>
              <p class="mb-4 flex-grow text-gray-600" v-html="(f.description || '').slice(0, 150) + (f.description && f.description.length > 150 ? '…' : '')"></p>
              <Link :href="route('formations.show', f.slug)" class="text-primary group-hover:text-primary-dark mt-auto inline-flex items-center self-start font-medium">
                Voir la formation
        ...
        <div v-if="formations.meta && formations.meta.last_page > 1" class="mt-12 flex justify-center">
          <nav class="flex items-center space-x-1">
            <Link v-for="link in formations.links" :key="link.label" :href="link.url || '#'" :class="['rounded-md border px-4 py-2 transition-colors', link.active ? 'bg-primary border-primary text-white' : link.url ? 'border-gray-300 text-gray-700 hover:bg-gray-50' : 'cursor-not-allowed border-gray-200 text-gray-400']">
