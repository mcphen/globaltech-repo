...
// Prefill personal info when a Lead/User is already authenticated via Inertia shared props
const page = usePage();
const prefilled = ref(false);

// Try multiple common locations/keys for the authenticated entity
const getProfileFromPage = (): any => {
    const props: any = page?.props || {};
    return (
        (props.lead ?? props.auth?.lead ?? null) ||
        (props.user ?? props.auth?.user ?? null) ||
        null
    );
};

const fillFromProfile = (p: any) => {
    if (!p || prefilled.value) return;

    // Support various possible field names
    const firstName = p.first_name ?? p.firstname ?? p.prenom ?? (typeof p.name === 'string' ? p.name.split(' ')[0] : undefined);
    const lastName = p.last_name ?? p.lastname ?? p.nom ?? (typeof p.name === 'string' ? p.name.split(' ').slice(1).join(' ') : undefined);
    const email = p.email ?? p.mail;
    const phone = p.phone ?? p.phone_number ?? p.mobile ?? p.telephone;
    const address = p.address ?? p.adresse ?? p.location ?? p.address_line;

    if (!form.first_name && firstName) form.first_name = firstName;
    if (!form.last_name && lastName) form.last_name = lastName;
    if (!form.email && email) form.email = email;
    if (!form.phone && phone) form.phone = String(phone);
    if (!form.address && address) form.address = address;

    prefilled.value = true;
};

// Fill immediately if available and also watch for late hydration
onMounted(() => {
    fillFromProfile(getProfileFromPage());
});

watch(
    () => getProfileFromPage(),
    (val) => {
        if (val) fillFromProfile(val);
    },
    { immediate: true }
);
